 ...
 */
let LiveEventProducers = new Map() //LiveEventProducers: key: eventId, and Value: producer username
/**
    ...
    socketServer.on('connection', async (socket) => {
        ...
        */
        socket.on('create_event', async (data, callback)=> {
            ...
            // console.log('create_event: producer data ', data)
        ...
        */
         socket.on('produce', async (data, callback) => {
            ...
            if ( kind === 'audio'){
                ...
                let eventProducer = EventProducerArr.filter(producer  => producer.username === liveEventProducerUsername)
                // console.log('@produce audio',eventProducer)
                let index = EventProducerArr.findIndex(producer  => producer.username === liveEventProducerUsername)
                // console.log('index: ', index)
                EventProducerArr[index].audioTrack = audioTrackProducer
        ...
        */
        socket.on('consume', async (data, callback) => { 
            ...
            if (!batchFlag){
               ...
               console.log('@consume live Event Producer username: ', eventProducer.username)
        ...
        */
          socket.on('haltliveStream', (data, callback) => {
            ...
            */
            ...
            /**
             * halt private live events
            */
            else{
                console.log('producerUsername: ', username)
                let producerTransport = findkeybyValue(username, LiveEventProducersTransport)
                console.log('producerTransportId: ', producerTransport)          
                try {
                    if (EventProducerArr){
                        console.log('EventProducerArr length: ', EventProducerArr.length)
            ...
            liveStreamUser.update(data1, condition, options)
            .then((resualt)=>{
                console.log(resualt)
            })
            .catch((err)=>{
            ...
            liveEventData.update(data1, conditionOne, options)
            .then((data)=>{
                console.log(data)
            })
            .catch((err)=>{
